...
%               -lambda: 
% 
%               Penalization parameter in the adaptive diffusion K-means.
%
%               -D:
%
...
function X=D_kmeans_sdp_adaptive(P, lambda, D, t, lazy)
...
%          D(ii,jj) = exp(-norm(P(:,ii)-P(:,jj))^2/2/gamma^2);
%     end
% end
 %D = rand(N) < D;
 DD = diag(sum(D)); % degree matrix
 D = DD \ D ; 
 D = (1-lazy) * D + lazy * eye(N);   % lazy random walk
 D = -D^t / DD;
 %D = -sqrt(DD) * D^500 / sqrt(DD);
 %D = ((diag(sum(D)) - D)/max(sum(D)))^100;

%SDP definition for SDPNAL+
n=N;
C{1}=D+lambda*n*eye(n);     % Penalized objective function
%C{1}=eye(n);                % lambda = \infty: result: 1/n*J_{n*n}
blk{1,1}='s'; blk{1,2}=n;
b=zeros(n,1);
Auxt=spalloc(n*(n+1)/2, n, 4*n);
idx=1;
for i=1:n
    A=zeros(n,n);
    A(:,i)=ones(n,1);
